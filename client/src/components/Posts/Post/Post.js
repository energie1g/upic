...
export default function(props) {
    ...
    useEffect(() => {
        ...
        async function fetchComments() {
            ...
            for (let i = 0; i < comments.length; i++) {
                let comment = axios.get(`/api/v1/comments/${comments[i]._id}`);
    ...
    const postComment = async () => {
        setBtnDisabled(true);
        const comm = await axios.post('/api/v1/comments', {
            comment,
            user: props.userId,
            image: props.id
        });
        ...
        <div className={classes.UpperPost}>
            ...
            <div className={classes.UpperPostInfo}>
                <div className={classes.UpperPostID}>
                    <span
                        className={classes.UpperPostName}
                        onClick={goToCompte}
                    >
                        {props.name}
                    </span>
                    ...
                    <span className={classes.UpperPostDate}>
                        &middot; {props.date}
                    </span>
                </div>
