...
class upload extends React.Component {
    ...
    Post = async e => {
        ...
        try {
            const image = await axios.post('/api/v1/images', formData);

            const user = await axios.get(`/api/v1/users/${this.state.user_id}`);

            image &&
                (await axios.patch(`/api/v1/users/${this.state.user_id}`, {
            ...
            if (error.response.data.message.multerError) {
                ...
                errorMessage =
                    error.response.data.message.errors.imgUrl.message;
            }
        }
